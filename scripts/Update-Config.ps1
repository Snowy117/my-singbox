...
foreach ($subscription in $runtime.subscriptions) {
    ...
    $tagMap = @{}
    foreach ($item in $items) { $tagMap[$item.tag] = "$($subscription.prefix)$($item.tag)" }
    foreach ($item in $items) {
        $item.tag = $tagMap[$item.tag]
        if ($item.PSObject.Properties['detour'] -and $tagMap.ContainsKey($item.detour)) { $item.detour = $tagMap[$item.detour] }
        if ($item.PSObject.Properties['outbounds']) {
            $item.outbounds = @($item.outbounds | ForEach-Object { if ($tagMap.ContainsKey($_)) { $tagMap[$_] } else { $_ } })
        }
    }
...
& (Join-Path $PSScriptRoot 'Build-Config.ps1') -NodesPath $nodesPath -OutputPath $candidate
